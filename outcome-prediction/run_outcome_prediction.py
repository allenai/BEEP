...
def test(model, dev_data, epoch=0, return_loss=False, class_weights=None, strategy='average'):
    ...
    if not return_loss and args.dump_test_preds:   # return_loss flag is not used for the test data
        pickle.dump(prediction_dict, open(os.path.join(args.out_dir, 'test_predictions.pkl'), 'wb'))
        pickle.dump(pred_prob_dict, open(os.path.join(args.out_dir, 'test_probabilities.pkl'), 'wb'))
...
if __name__ == '__main__':
    ...
    model = BertLongForSequenceClassification.from_pretrained(model_path, config=config) if args.longmodel_dir is not None \
            else BertForSequenceClassification.from_pretrained(model_path, config=config)
    ...
    if args.lit_dir is not None and args.doc_embeds is None:
        if args.enc_strategy == 'bienc':
            model = LitAugPredictorBienc(config, model, args.num_top_docs, args.strategy)
        elif args.enc_strategy == 'crossenc':
            model = LitAugPredictorCrossenc(config, model, args.num_top_docs, args.strategy)
    if args.lit_dir is not None and args.doc_embeds is not None:
        if args.query_proj is None:
            model = L2RLitAugPredictorBienc(config, model, args.l2r_top_docs, args.strategy, rerank_model)
        else:
            model = L2RLitAugPredictorBienc(config, model, args.l2r_top_docs, args.strategy, rerank_model, args.query_proj)
    ...
    def preprocess_function(examples, split):
        ...
        if args.lit_dir is not None and args.doc_embeds is None:
            ...
            k_range = int(args.num_top_docs) if args.num_top_docs >= 1 else max([len(x[-1]) for x in examples])
            if k_range > 0:
                if args.enc_strategy == 'bienc':
    ...
    train_batches = batch_and_tokenize_data(dataset.train_data, args.batch_size, 'train')
    print('Created {} train batches'.format(len(train_batches)))
    dev_batches = batch_and_tokenize_data(dataset.dev_data, args.batch_size, 'dev')
    print('Created {} dev batches'.format(len(dev_batches)))
    test_batches = batch_and_tokenize_data(dataset.test_data, args.batch_size, 'test')
    print('Created {} test batches'.format(len(test_batches)))

    if args.do_train:
        train(model, train_batches, dev_batches, args.out_dir, args.epochs, args.lr, dataset.class_weights, args.accumulation_steps, args.strategy)
    if args.do_test:
